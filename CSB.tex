...
\usepackage{pgfplots}
\usepackage{tikz}
\usetikzlibrary{arrows, matrix, backgrounds, calc, positioning}
...
\begin{example}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw[semithick, -latex] (X) ++ (-15mm,0) -- ++(5mm,0)
        node (Fu) {} node[below] {$R_1$} -- (X);

      \draw[semithick, -latex] (X) -- node[midway, above] {$R_2$} ++(2,0)
        coordinate (FY);

      \draw[semithick, -left to] ($(Yp.east)+(0,.05)$) -- 
      ...
      \draw[semithick, densely dashed, -latex] (X) -- (FX);
      \draw[semithick, densely dashed, -latex] (Y) -- (FY -| Y);
      \draw[semithick, densely dashed, -latex] (Fu) ++(-1, 5mm)
        node[left] {$u$} -| (Fu);
    \end{tikzpicture}
  \end{center}
...
\begin{example}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw[semithick, -latex] (X) ++ (-15mm,0) -- ++(5mm,0)
        node (Fu) {} -- node[midway, above] {$R_1$} (X);

      \draw[semithick, -latex] (X) -- node[midway, above] {$R_3$} ++(2,0);

      \draw[semithick, -left to] ($(Y.east)+(0,.05)$) -- 
        ($(Y.east)!0.5!(Yp.west) + (0,.05)$) node (FX) {} -- 
        node[pos=.6, above] {$R_5$} ($(Yp.west)+(0,.05)$);

      \draw[semithick, left to-] ($(Y.east)-(0,.05)$) -- 
        node[pos = .6, below] {E}
        node[pos = .2, below] {$R_4$} ($(Yp.west)-(0,.05)$);

      \draw[semithick, densely dashed, -latex] (X) -- (FX);
      \draw[semithick, densely dashed, -latex] (Y) -- node[midway, left] {$R_2$} (Fu -| Y);
      \draw[semithick, densely dashed, -latex] (Fu) ++(-1, 5mm)
        node[left] {$u$} -| (Fu);
    \end{tikzpicture}
  \end{center}
...
\begin{example}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw[semithick, -left to] ($(Y.east)+(0,.05)$) --
        node[midway] (FY) {}
        node[pos=.8, above] {$R_5$}
        ($(Yp.west)+(0,.05)$);

      \draw[semithick, left to-] ($(Y.east)-(0,.05)$) --
        node[midway, below] {E} 
        node[pos=.2, below] {$R_4$}
        ($(Yp.west)-(0,.05)$);

      \draw[semithick, -latex] (Z) ++(-15mm,0) -- node[midway] (FZ) {} (Z);
      \draw[semithick, -latex] (Z) -- node[near end, below] {$R_7$} ++(15mm,0);

      \node (FXp) at (FX -| Y) {};
      \node (FYpp) at (FY -| X) {};
      \node (FZp) at (FZ -| Y) {};

      \draw[semithick, densely dashed, -latex] (Y) -- node[midway, left] {$R_2$} (FXp);
      \draw[semithick, densely dashed, -latex] (Y) -- node[midway, left] {$R_6$} (FZp);
      \draw[semithick, densely dashed, -latex] (X) -- (FYpp);

      \draw[semithick, densely dashed, -latex] (FX) ++(-10mm, 5mm) node[left] {$u$} -| (FXp);
      \draw[semithick, densely dashed, -latex] (Z) to[out=30, in=-60, looseness=2] (FXd);
